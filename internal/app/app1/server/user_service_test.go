...
func TestUserServer_ForgotPasswordFailure1(t *testing.T) {
	ctx := context.TODO()
	server := NewUserServer(mysql.NewEntClient(), rpcauthorization.NewAuthorizationMock(nil), username, password, mail.MockFail, rpcspaces.NewSpacesMock(nil))
	server.users.DeleteAll(ctx)
	username := "simo"
	user,_ := server.CreateUser(ctx, &rpcusers.CreateUserParams{
		Username: username,
...
func TestUserServer_VerifyResetPasswordExpired(t *testing.T) {
	ctx := context.TODO()
	server := NewUserServer(mysql.NewEntClient(), rpcauthorization.NewAuthorizationMock(nil), username, password, mail.Mock, rpcspaces.NewSpacesMock(nil))
	server.users.DeleteAll(ctx)
	user,_ := server.CreateUser(ctx, &rpcusers.CreateUserParams{
		Username: "simo",
		Password: "simo",
